...
#LUT3 = Generator3DLUT_zero(dim=opt.dim)
#LUT4 = Generator3DLUT_zero(dim=opt.dim)
...
def generate_LUT(img, fix_pred=None):
    ...
    return LUT, 1 # pred.detach().to('cpu').numpy()
...
if not opt.video:
    ...
    if os.path.isdir(opt.image_path):
        ...
        frames = open(opt.image_path, 'r').read().split('\n')
    else:
        frames = [opt.image_path]

    # preds = []
    fix_pred = None # torch.tensor([ 1.1667712, -1.1818283, -0.7892431]).cuda()
    for frame in tqdm(frames):
        ...
        # cv2.imwrite(os.path.join(opt.output_dir, os.path.basename(frame)), output)
...
# np.save('{}.npy'.format(os.path.basename(opt.image_path)), preds)
