...
parser.add_argument("--dim", type=int, default=33, help="dimension of LUT")
...
# LUT1 = Generator3DLUT_zero(dim=opt.dim)
# LUT2 = Generator3DLUT_zero(dim=opt.dim)
...
optimizer_G = torch.optim.Adam(itertools.chain(LUT0.parameters()), lr=opt.lr, betas=(opt.b1, opt.b2)) #, LUT3.parameters(), LUT4.parameters()
...
def generator_train(img):
    ...
    for b in range(img.size(0)):
        combine_A[b,:,:,:] = gen_A0[b,:,:,:] #+ pred[b,1] * gen_A1[b,:,:,:] + pred[b,2] * gen_A2[b,:,:,:] #+ pred[b,3] * gen_A3[b,:,:,:] + pred[b,4] * gen_A4[b,:,:,:]

    return combine_A, 1 # weights_norm

def generator_eval(img):

    # pred = classifier(img).squeeze()
    
    LUT = LUT0.LUT # pred[0] * LUT0.LUT + pred[1] * LUT1.LUT + pred[2] * LUT2.LUT #+ pred[3] * LUT3.LUT + pred[4] * LUT4.LUT
...
def calculate_psnr():
    ...
    for i, batch in enumerate(psnr_dataloader):
        ...
        l1 = torch.nn.L1Loss(fake_B * mask, real_B * mask)
        # l1 = l1_loss(fake_B, real_B)
        # psnr =10 * math.log10(255.0 * 255.0 / mse.item())
        
        avg_loss += l1

    return avg_loss/ len(psnr_dataloader)
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        if not opt.loss_weight:
            ...
            mask = Variable(batch["mask"].type(Tensor))
            l1 = l1_loss(fake_B * mask, real_B * mask)
        ...
        # weights_norm is for CNN regularzation, thus removed for single LUT 
        loss = mse + opt.lambda_smooth * tv_cons + opt.lambda_monotonicity * mn_cons
        # loss = mse + opt.lambda_smooth * tv_cons
        # loss = mse 
        # loss = l1  + opt.lambda_smooth * tv_cons + opt.lambda_monotonicity * mn_cons

        loss_avg += loss #10 * math.log10(1 / mse.item())
        ...
        # Print log
        sys.stdout.write(
            "\r[Epoch %d/%d] [Batch %d/%d] [loss: %f, tv: %f, mn: %f] ETA: %s"
            % (epoch,opt.n_epochs,i,len(dataloader),loss_avg / (i+1),tv_cons, mn_cons, time_left,
            )
        )

    sys.stdout.write(
        "[Epoch %d/%d] [Batch %d/%d] [loss: %f, tv: %f, mn: %f] ETA: %s"
        % (epoch,opt.n_epochs,i,len(dataloader),loss_avg / (i+1),tv_cons, mn_cons, time_left,
        )
    )
    # avg_loss = calculate_psnr()
    # if avg_loss < min_loss:
    #     min_loss = avg_loss
    #     max_epoch = epoch
    # sys.stdout.write(" [LOSS: %f] [min LOSS: %f, epoch: %d]\n"% (avg_loss, min_loss, max_epoch))
    ...
    if epoch % opt.checkpoint_interval == 0:
        ...
        # file.write(" [LOSS: %f] [min LOSS: %f, epoch: %d]\n"% (avg_loss, min_loss, max_epoch))
        file.write("[Epoch %d/%d] [Batch %d/%d] [loss: %f, tv: %f, mn: %f]"
            % (epoch,opt.n_epochs,i,len(dataloader),loss_avg / (i+1),tv_cons, mn_cons,
            ))
        file.close()
